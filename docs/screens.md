...
| Ficha de obra | `/obras/[id]` | La obra, tu entrada y lo que ha hecho el grupo con ella. Secciones condicionales: expansiones si es un juego de mesa base (ADR-0006), mazos propios si es TCG (ADR-0007). El `[id]` es el de la **entrada**, no el de la obra: la pantalla lee `GET /v1/library/{id}`, que ya trae el `Work` dentro. Un 404 solo lo produce `library_entry_not_found` (que cubre a la vez «no existe» y «no es tuya», a propósito); cualquier otro fallo es un estado de la pantalla, nunca un 404. «Tu entrada» se edita: estado (solo las transiciones que dibuja `domain.md`), progreso, valoración (solo en `completed` y `dropped`), favorito y propiedad, con un `PATCH` que envía solo lo que cambia; y se puede quitar la entrada, con confirmación, porque el `DELETE` no es idempotente. La nota y las dos fechas siguen en solo lectura |
